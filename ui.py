...
class LoginWindow(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("StylModa v3 — production release")
    ...
    def _build(self):
        ...
        tk.Label(left, text="Production-релиз после β-тестирования", bg=ACCENT_DARK, fg="#E8F0E0", font=("Segoe UI", 13)).pack(anchor="w", padx=36)
        tk.Label(
            left,
...
class MainWindow(tk.Tk):
    def __init__(self, user):
        super().__init__()
        self.user = user
        self.title(f"StylModa v3 — {user['full_name']}")
    ...
    def _build(self):
        ...
        ttk.Label(header, text="StylModa v3", style="Title.TLabel").pack(side="left")
    ...
    def _build_alpha_tab(self):
        ...
        self.alpha_tree = self._make_tree(
            ...
            ("ID", "Проверка клиента", "Проблема", "Решение в системе", "Результат", "Подтв."),
    ...
    def _build_admin_dashboard_tab(self):
        ...
        ttk.Label(tab, text="Административная панель production v3", style="Section.TLabel").pack(anchor="w", pady=(10, 6))
        cards = ttk.Frame(tab)
