...
class AuthService:
    ...
    def register_buyer(self, username: str, password: str, full_name: str, phone: str, email: str):
        ...
        if not re.fullmatch(r"[a-zA-Z0-9_.-]{3,30}", username):
            raise BusinessError("Логин должен содержать 3–30 символов: латиница, цифры, точка, дефис или подчёркивание.")
        if len(password) < 6:
        ...
        if phone and not (10 <= len(digits) <= 15):
            raise BusinessError("Телефон должен содержать от 10 до 15 цифр.")
        if email and not re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", email):
            raise BusinessError("Введите корректный e-mail или оставьте поле пустым.")
...
class OrderService:
    def checkout(self, user, delivery_type, address, comment):
        ...
        if delivery_type == "доставка" and len(address) < 10:
            raise BusinessError("Для доставки укажите адрес не короче 10 символов.")
        if delivery_type == "самовывоз" and address:
            # Адрес не обязателен для самовывоза, но может использоваться как комментарий к точке выдачи.
            address = address[:120]
        if len(comment) > 250:
            raise BusinessError("Комментарий к заказу не должен превышать 250 символов.")
        now = datetime.now().isoformat(timespec="seconds")
        with get_connection() as conn:
            items = conn.execute(
